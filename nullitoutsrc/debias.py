...
def debias_by_means_all_classes(X_train: np.ndarray, Y_train:np.ndarray, input_dim:int, by_class=False, Y_train_main=None, neutral_class = 0):
    relevant_idx_train = np.ones(X_train.shape[0], dtype=bool)
    vectors = []
    if by_class:
        if (Y_train_main is None):
        ...
        # print("The chosen class is {}".format(cls))
        # relevant_idx_train = Y_train_main == cls
        for i in range(len(main_task_labels)):
            cls = main_task_labels[i]
            relevant_idx_train = Y_train_main == cls
            X_train_subset = X_train[relevant_idx_train]
            Y_train_subset = Y_train[relevant_idx_train]

            classes = np.unique(Y_train_subset)
            separated = {c: X_train_subset[Y_train_subset == c] for c in classes}
            means = {c: np.mean(separated[c], axis=0).reshape(1,-1) for c in classes}
            if len(classes) > 2:
                vectors.append({"{} - {}".format(neutral_class, c): means[neutral_class] - means[c] for c in classes if c != neutral_class})
            else:
                c1, c2 = classes
                vectors.append(means[c1] - means[c2])
    else:
        classes = np.unique(Y_train)
        separated = {c: X_train[Y_train == c] for c in classes}
        means = {c: np.mean(separated[c], axis=0).reshape(1,-1) for c in classes}
        if len(classes) > 2:
            vectors.append({"{} - {}".format(neutral_class, c): means[neutral_class] - means[c] for c in classes if c != neutral_class})
        else:
    ...
    P, rowspace_projections = debias_by_specific_directions(vectors, input_dim=input_dim)
    return P, rowspace_projections


def debias_by_means_a_random_class(X_train: np.ndarray, Y_train:np.ndarray, input_dim:int, by_class=False, Y_train_main=None, neutral_class = 0):
    relevant_idx_train = np.ones(X_train.shape[0], dtype=bool)
    vectors = []
    if by_class:
        if (Y_train_main is None):
...
def get_debiasing_projection(classifier_class, cls_params: Dict, num_classifiers: int, input_dim: int,
    ...
    rowspace_projections = []
    Ws = []
    Ws_stacked = np.empty((0, input_dim), dtype="float64")
    ...
    for i in pbar:
        ...
        accuracy_scores.append(acc * 100)

        if acc < min_accuracy: continue

        W = clf.get_weights()
        Ws.append(W)
        Ws_stacked = np.vstack((Ws_stacked, W.reshape((-1, input_dim))))
        ...
        if is_autoregressive:
            ...
            P =  get_rowspace_projection(Ws_stacked)
            P = I - P
            # project
